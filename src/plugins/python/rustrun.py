...
def run_content_in_docker(content, *, normal_timeout=2, compile_timeout=10, run_timeout=5):
    ...
    try:
        ...
        p = subprocess.run(f'sudo -S docker cp ./tmp.rs {name}:/root/tmp.rs', stdout=subprocess.PIPE, stderr=subprocess.STDOUT, encoding="utf-8", shell=True, timeout=normal_timeout)
        # print(p.stdout.rstrip())
        ...
        p = subprocess.run(f'sudo -S docker exec {name} bash -c "/root/.cargo/bin/rustc /root/tmp.rs -o /root/tmp 2>&1"', stdout=subprocess.PIPE, stderr=subprocess.STDOUT, encoding="utf-8", shell=True, timeout=compile_timeout)
        res = p.stdout.rstrip()
        result += res
        result = result.strip()
        prefix = "run"
