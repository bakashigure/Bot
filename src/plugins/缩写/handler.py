...
query = on_command("缩写", aliases={"拼音", "查拼音", "查缩写"})
submit = on_command("提交缩写")
...
def get_trans_result_str(trans: list[dict, ...]) -> str:
    ...
    for word in trans:
        ...
        if consts.QUERY_KEY_TRANS in word:
            ...
            s += f':\n 尚未录入，可以使用"提交缩写 {word["name"]} <含义>"提交新的含义，含义的末尾可用括号注明来源'
        result.append(s)
    return '\n'.join(result)
...
async def handle_query(event: MessageEvent, args: Message = CommandArg()):
    ...
    try:
        ...
        await query.finish(reply_text(f'查询出错！错误信息：{exc}', event))
...
async def handle_submit(matcher: Matcher, event: MessageEvent, args: Message = CommandArg()):
    ...
    if len(raw_args) != 2:
        await query.finish(reply_text('提交词条的参数数量错误', event))
...
@submit.got("confirm", prompt="您确认要提交该词条吗，缩写将提交至 https://lab.magiconch.com/nbnhhsh/（输入Y确认）")
async def handle_submit_confirm(
    ...
    if confirm.extract_plain_text().upper() == 'Y':
        try:
            ...
            await query.finish(reply_text(f'提交出错！错误信息：{exc}', event))
        await submit.finish(reply_text(f'成功提交 {name}: {trans} 至 nbnhhsh，审核通过后此词条将会生效', event))
